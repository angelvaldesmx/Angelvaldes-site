document.addEventListener('DOMContentLoaded', async () => {
  const body=document.body,
        ...
        blogContentModal=document.getElementById("blogContentModal"),
  ...
  async function fetchArticles(){
    try{const res=await fetch('/articulos.json');return await res.json();}
    catch(err){console.error('Error cargando JSON:',err);return {destacados:[],recientes:[],semanales:[],mensuales:[]};}
  }
  ...
  function createArticleElement(article){
    ...
    div.querySelector('.read-more-link').addEventListener('click',e=>{
      e.preventDefault(); openArticleModal(article); 
  ...
  function openArticleModal(article){
    ...
    // Anuncios dinámicos
    document.getElementById('article-ads-container-top').innerHTML=`<iframe src="${article.adTop||'about:blank'}" loading="lazy"></iframe>`;
    document.getElementById('article-ads-container-bottom').innerHTML=`<iframe src="${article.adBottom||'about:blank'}" loading="lazy"></iframe>`;
  ...
  function openListModal(title,list){
    ...
    list.forEach(item=>{
      ...
      a.addEventListener('click',e=>{
        e.preventDefault(); openArticleModal(item); history.pushState({slug:item.slug},item.title,`/articulos/${item.slug}`);
      });
  ...
  ["openWeekly","openWeeklySidebar"].forEach(id=>{
    document.getElementById(id)?.addEventListener('click', async e=>{
      e.preventDefault(); openListModal("Blogs Semanales",(await fetchArticles()).semanales);
    });
  });

  ["openMonthly","openMonthlySidebar"].forEach(id=>{
    document.getElementById(id)?.addEventListener('click', async e=>{
      e.preventDefault(); openListModal("Blogs Mensuales",(await fetchArticles()).mensuales);
    });
  });

  // Read more link delegation
