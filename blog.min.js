document.addEventListener('DOMContentLoaded', async () => {
  ...
  function createArticleElement(article) {
    ...
    div.innerHTML = `
      ...
      <a href="/articulos/${article.slug}" class="read-more-link" data-article-id="${article.id}">Leer más</a>
    `;
  ...
  function openArticleModal(article){
    ...
    document.title = `${article.title} | Mi Blog Creativo`;
    ...
    metaDesc.content = article.subtitle || article.content.slice(0,150);
    ...
    linkCanonical.href = `https://angelvaldesmx.qzz.io/articulos/${article.slug}`;

    let ldJson = document.getElementById("article-jsonld");
    if(!ldJson){
      ldJson = document.createElement("script");
      ldJson.type = "application/ld+json";
      ldJson.id = "article-jsonld";
    ...
    ldJson.textContent = JSON.stringify({
      ...
      "url": `https://angelvaldesmx.qzz.io/articulos/${article.slug}`,
      "mainEntityOfPage": { "@type":"WebPage", "@id": `https://angelvaldesmx.qzz.io/articulos/${article.slug}` }
    });
  }
  ...
  // Delegación para links internos
  ...
  const pathMatch = window.location.pathname.match(/\/articulos\/([^\/]+)/);
  if(pathMatch){
    const initialSlug = pathMatch[1];
    const art = articles.find(a => a.slug === initialSlug);
    if(art) openArticleModal(art);
  }
});    const div = document.createElement('div');
    div.className = 'blog-card';
    // Usamos hash por compatibilidad con /blog
    div.innerHTML = `
      ${article.image ? `<img src="${article.image}" alt="${article.title}" loading="lazy">` : ''}
      <h3>${article.title}</h3>
      <p>${article.subtitle || ''}</p>
      <a href="#${article.slug}" class="read-more-link" data-article-id="${article.id}">Leer más</a>
    `;
    div.querySelector('.read-more-link').addEventListener('click', e => {
      e.preventDefault();
  ...
  function openArticleModal(article, pushState = false){
    modalArticleTitle.textContent = article.title;
    modalArticleImage.src = article.image || '';
    const p = article.content.split('\n\n');
    modalArticleText1.innerHTML = p[0] || '';
  ...
  document.body.addEventListener('click', e=>{
    ...
    if(link){
      e.preventDefault();
      const article = allArticles.find(a=>a.id == link.dataset.articleId);
      if(article) openArticleModal(article, true);
    }
  });

  // Hash para compatibilidad con /blog
  ...
  // Popstate para URLs limpias /articulos/slug
  window.addEventListener('popstate', ()=>{
    const slug = history.state?.slug;
    if(slug){
      const article = allArticles.find(a=>a.slug == slug);
  ...
  // Apertura directa al cargar página
  const pathMatch = window.location.pathname.match(/\/articulos\/([^\/]+)/);
  if(pathMatch){
    const initialSlug = pathMatch[1];
    const art = articles.find(a => a.slug === initialSlug);
  ...
  // Apertura directa por slug al cargar
  const pathMatch = window.location.pathname.match(/\/articulos\/([^\/]+)/);
  if(pathMatch){
    const initialSlug = pathMatch[1];
    const art = articles.find(a => a.slug === initialSlug);
  ...
  function openArticleModal(article){
    ...
    linkCanonical.href = `https://angelvaldesmx.qzz.io/blog#${article.slug}`;
